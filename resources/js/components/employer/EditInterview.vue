                ...
                <div class="row push">
                    ...
                    <div class="col-lg-12 col-xl-12">
                        <div class="form-group" >
                            ...
                            <v-select label="title" :clearable="false" :options="vacancies" v-model="vacancy"  ></v-select>
                            ...
                            <tbody>
                                <tr class="" v-for="(question, index) in questions" :key="index">
                                    ...
                                    <td class="font-w600" style="width:450px;">
                                        <textarea v-model="question.question" class="form-control" id="example-textarea-input" name="example-textarea-input" rows="1" placeholder="Введите текст вопроса"></textarea>
                                    </td>
                            ...
                            <tbody>
                                <tr class="" v-for="(quiz, index) in quizzes" :key="index">
                                    ...
                                    <td class="font-w600" style="width:250px;">
                                        <textarea v-model="quiz.question" class="form-control" id="example-textarea-input" name="example-textarea-input" rows="1" placeholder="Введите текст вопроса"></textarea>
                                    </td>
                                    <td style="width: 150px;">
                                        ...
                                        <input type="text" v-model="quiz.correct" class="form-control"  name="example-text-input" placeholder="Ответ №4">
                                    </td>                                    
                                    <td class=" text-muted" style="width: 80px;">
...
const mustBeSelected = (vacancy) => vacancy.id !== null
...
export default {
    ...
    axios.get(`/api/employer/interview/get/`+this.interview_id).then(({ data }) => {
            if (data) {
            this.questions = data.questions;
            this.quizzes = data.quizzes;
            this.vacancy = {id:data.vacancy_id, title: data.title};
    ...
    methods: {
        ...
        save(){
            ...
            axios.post('/api/employer/interview/update', {interview_id:this.interview_id, vacancy: this.vacancy.id, title:this.vacancy.title, description: this.description,  user_id: this.user_id, questions: this.questions, quizzes: this.quizzes}
                ).then(res => {                    
                    loader.hide()
                    this.$swal('Спасибо!', 'Интервью успешно изменён!', 'success');
                    setTimeout(document.location.reload(true), 1000);
                }).catch(err => {
                    loader.hide()
                    console.log(err)
                    this.$swal('Ошибка', 'Произошла ошибка при изменение интервью' + err, 'error');
                });
        }
  }
}
